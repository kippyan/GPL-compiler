...
#ifdef SYM_TAB
static Table_handler& table_handler = Table_handler::instance();
#endif
...
int main(int argc, char **argv)
{
  ...
  string filename_with_extension=filename;
  if (!yyin)
  {
    filename_with_extension+=".gpl";
  ...
  // if -DGRAPHICS was specified when compiling gpl.cpp then include this code
    ...
    symbol = table_handler.lookup("window_x");
    if (symbol != nullptr)
    {
      ...
      else window_x = symbol->as_integer();
    }
    symbol = table_handler.lookup("window_y");
    if (symbol != nullptr)
    {
      if (symbol->get_type() != INT)
        Error::error(Error::INVALID_TYPE_FOR_RESERVED_VARIABLE,
      ...
      else window_y = symbol->as_integer();
    }
    symbol = table_handler.lookup("window_width");
    if (symbol != nullptr)
    {
      if (symbol->get_type() != INT)
        Error::error(Error::INVALID_TYPE_FOR_RESERVED_VARIABLE,
      ...
      else window_width = symbol->as_integer();
    }
    symbol = table_handler.lookup("window_height");
    if (symbol != nullptr)
    {
      if (symbol->get_type() != INT)
        Error::error(Error::INVALID_TYPE_FOR_RESERVED_VARIABLE,
      ...
      else window_height = symbol->as_integer();
    }
    symbol = table_handler.lookup("window_red");
    if (symbol != nullptr)
    {
    ...
    symbol = table_handler.lookup("window_green");
    if (symbol != nullptr)
    {
      if (symbol->get_type() != DOUBLE)
        Error::error(Error::INVALID_TYPE_FOR_RESERVED_VARIABLE,
    ...
    symbol = table_handler.lookup("window_blue");
    if (symbol != nullptr)
    {
      if (symbol->get_type() != DOUBLE)
        Error::error(Error::INVALID_TYPE_FOR_RESERVED_VARIABLE,
    ...
    symbol = table_handler.lookup("window_title");
    if (symbol != nullptr)
    {
    ...
    symbol = table_handler.lookup("animation_speed");
    if (symbol != nullptr)
    {
      if (symbol->get_type() != INT)
        Error::error(Error::INVALID_TYPE_FOR_RESERVED_VARIABLE,
      ...
      else animation_speed = symbol->as_integer();
    }
  #endif
