...
std::map<const std::string, Gpl_type> Game_object::var_types//tagger
{
  //Game_object
  {"x",               INT},
  {"y",               INT},
...
Game_object::Game_object(bool draw_me, double red /* =  0.5 */,
                         double green /* = 0.5 */,
                         double blue /* =  0.5 */
                         ) : draw_me(draw_me)
{
...
{
  auto typed_void_ptr = lookup_registered_member_variable(name);
  return typed_void_ptr != nullptr;
}

//tagger. 
//If the name is not a valid member variable, an out_of_range exception is thrown
/*static*/ Gpl_type Game_object::get_member_variable_type(const std::string& name)
{
  return var_types.at(name);
}

void Game_object::set_member_variable(string name, int value)
{
  graphics_dirty = true;
  auto variable = lookup_registered_member_variable(name);
  if (!variable)
      throw MEMBER_NOT_DECLARED;
  if (variable->m_type != INT)
    throw MEMBER_NOT_OF_GIVEN_TYPE;
...
void Game_object::set_member_variable(string name, double value)
{
  graphics_dirty = true;
  auto variable = lookup_registered_member_variable(name);
  if (!variable)
...
void Game_object::set_member_variable(string name, string value)
{
  graphics_dirty = true;
  auto variable = lookup_registered_member_variable(name);
  if (!variable)
...
void Game_object::set_member_variable(string name, Animation_block *value)
{
  graphics_dirty = true;
  auto variable = lookup_registered_member_variable(name);
  if (!variable)
...
void Game_object::get_member_variable(string name, int &value)
{
  auto variable = lookup_registered_member_variable(name);
  if (!variable)
    throw MEMBER_NOT_DECLARED;
...
void Game_object::get_member_variable(string name, double &value)
{
  auto variable = lookup_registered_member_variable(name);
  if (!variable)
    throw MEMBER_NOT_DECLARED;
  ...
  else throw MEMBER_NOT_OF_GIVEN_TYPE;
}

void Game_object::get_member_variable(string name, string &value)
{
  auto variable = lookup_registered_member_variable(name);
  if (!variable)
    throw MEMBER_NOT_DECLARED;
...
void Game_object::get_member_variable(string name, Animation_block *&value)
{
  auto variable = lookup_registered_member_variable(name);
  if (!variable)
    throw MEMBER_NOT_DECLARED;
...
Game_object::Typed_void_ptr::~Typed_void_ptr()
{
}

std::shared_ptr<Game_object::Typed_void_ptr> Game_object::lookup_registered_member_variable(string name)
{
    // Check if name in map of registered_member_variables
    //map<string, Typed_void_ptr *>::iterator iter;
    auto iter = m_variable_registry.find(name);
                                          ...
                                          )
{
    ...
    m_variable_registry[name] = std::make_shared<Typed_void_ptr>(type, value, name);
}
...
std::ostream& operator<<(std::ostream& os, const Game_object& go)
{
  ...
  auto iter=go.m_variable_registry.begin();
  for (; iter != go.m_variable_registry.end(); iter++)
  {
    auto cur_member = (*iter).second;
    string cur_name = (*iter).first;

    if (iter != go.m_variable_registry.begin())
      os << ",\n";
    os << "    " << cur_name << " = ";
