...
void Member_variable::assign(Constant* cptr){
  switch (symbol->get_type){
    case INT:
      symbol->as_int() = cptr->as_int();
      break;
    case DOUBLE:
      symbol->as_double() = cptr->as_double();
      break;
    case STRING:
      symbol->as_string() = cptr->as_string();
      break;
    case ANIMATION_BLOCK:
      symbol->as_Animation_block = cptr->as_animation_block();
      break;
    default:
      std::cerr << "operator called on invalid Member_variable type\n";
      break;
  }
}
void Member_variable::plus_equals(Constant* cptr){
  switch (symbol->get_type){
    case INT:
      symbol->as_int() = cptr->as_int();
      break;
...
void Member_variable::minus_equals(Constant* cptr){
  switch (symbol->get_type){
    case INT:
      symbol->as_int() = cptr->as_int();
      break;
...
void Member_variable::plus_plus(){
  switch (symbol->get_type){
    case INT:
      symbol->as_int()++;
      break;
...
void Member_variable::minus_minus(){
  switch (symbol->get_type){
    case INT:
      symbol->as_int()--;
      break;
